        ...
        if (pClient->connect(sensorAddress)) {
            ...
            if (pRemoteService != nullptr) {
                ...
                if (pRemoteCharacteristic != nullptr && pRemoteCharacteristic->canNotify()) {
                    // Capture 'this' explicitly to pass instance
                    pRemoteCharacteristic->registerForNotify([this](BLERemoteCharacteristic* pRemoteCharacteristic, uint8_t* pData, size_t length, bool isNotify) {
                        notifyCallback(pRemoteCharacteristic, pData, length, isNotify, this);
                    });
                    connected = true;
...
// Static function updated to accept instance
void HLKLD2450Lib::notifyCallback(BLERemoteCharacteristic* pRemoteCharacteristic, uint8_t* pData, size_t length, bool isNotify, HLKLD2450Lib* instance) {
    // Use instance to access dataBuffer
    for (size_t i = 0; i < length; i++) {
        instance->dataBuffer.push_back(pData[i]);
    }
}
